...
c.execute('''CREATE TABLE contacts
             ...
             Secondary_Email TEXT, Tertiary_Email TEXT, LinkedIn_URL TEXT, Mobile TEXT, Work_Phone TEXT, Country TEXT)''')
conn.commit()
...
with open(input_file, 'r') as infile:
    for line in infile:
        ...
        if contact_data:
            data_to_insert.append((contact_data['First_Name'], contact_data['Last_Name'], contact_data['Email'],
                                   contact_data['Secondary_Email'], contact_data['Tertiary_Email'],
                                   contact_data['LinkedIn_URL'],
                                   contact_data['Mobile'], contact_data['Work_Phone'], contact_data['Country']))
            record_counter += 1

            if record_counter % bulk_insert_size == 0:
                c.executemany('''INSERT INTO contacts (First_Name, Last_Name, Email, Secondary_Email,
    ...
    if data_to_insert:
        c.executemany('''INSERT INTO contacts (First_Name, Last_Name, Email, Secondary_Email,
                         ...
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', data_to_insert)
        conn.commit()
        print(f"Оставшиеся {len(data_to_insert)} записей добавлены в базу данных.")
